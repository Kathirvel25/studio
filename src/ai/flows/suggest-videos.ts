...
const SuggestVideosOutputSchema = z.object({
  videos: z.array(VideoSuggestionSchema).describe('A list of suggested YouTube videos.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant that helps students find relevant YouTube videos for their studies. Based on the subject provided, suggest 5 helpful YouTube videos. For each video, provide a potential title, a likely YouTube channel, and a brief reason for the recommendation.

Subject: {{{subject}}}

Generate a list of 5 video suggestions.`,
});
